...
import { motion, Reveal, StaggerGroup, fadeUp } from "@/components/motion";
import {
...
export default function DashboardPage() {
  ...
  const StatCard = ({ icon: Icon, label, value, numericValue, sub, color = "violet" }: {
    icon: React.ElementType; label: string; value: string | number; numericValue?: number; sub?: string; color?: string;
  }) => {
    ...
    return (
      <motion.div variants={fadeUp} whileHover={{ y: -2 }} className={`glass bg-gradient-to-br ${colorMap[color]} p-5 rounded-2xl transition-shadow hover:shadow-lg`}>
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <Reveal delay={0.05} className="glass p-5 rounded-2xl bg-gradient-to-br from-emerald-500/10 via-transparent to-transparent border-emerald-500/15">
      ...
      <StaggerGroup className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={ShieldCheck} label="Evaluated" value={stats?.total_evaluated ?? 0} numericValue={stats?.total_evaluated ?? 0} sub="Total transactions" color="violet" />
        <StatCard icon={TrendingUp} label="Allowed" value={stats?.total_allowed ?? 0} numericValue={stats?.total_allowed ?? 0} sub={`${stats ? ((stats.total_allowed / (stats.total_evaluated || 1)) * 100).toFixed(1) : 0}% approval rate`} color="green" />
        <StatCard icon={AlertTriangle} label="Blocked" value={stats?.total_blocked ?? 0} numericValue={stats?.total_blocked ?? 0} sub={`${stats ? ((stats.total_blocked / (stats.total_evaluated || 1)) * 100).toFixed(1) : 0}% block rate`} color="red" />
      ...
      <StaggerGroup className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard icon={Activity} label="In Review" value={stats?.total_reviewed ?? 0} numericValue={stats?.total_reviewed ?? 0} sub="Awaiting human" color="amber" />
      ...
      {/* Live feed */}
      <Reveal delay={0.15} className="glass p-5 rounded-2xl">
        <div className="flex items-center justify-between mb-4">
